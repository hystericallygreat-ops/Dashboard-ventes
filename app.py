...
if is_admin:
    st.sidebar.markdown("### 📂 Gestion fichier")

    uploaded_file_admin = st.sidebar.file_uploader("Remplacer fichier Excel", type=["xlsx"])

    if uploaded_file_admin:
        with open(SAVE_PATH, "wb") as f:
            f.write(uploaded_file_admin.getbuffer())
        st.sidebar.success("✅ Fichier remplacé")
        st.rerun()

    if os.path.exists(SAVE_PATH):
        if st.sidebar.button("🗑 Supprimer le fichier actuel"):
            os.remove(SAVE_PATH)
            st.sidebar.warning("Fichier supprimé")
            st.rerun()
...
if not os.path.exists(SAVE_PATH):
    st.info("🔒 Aucun fichier chargé. Connecte-toi en admin pour en uploader un.")
    st.stop()

try:
    xls = pd.ExcelFile(SAVE_PATH)

    required_sheets = ["Extraction", "Code", "Objectifs"]
    if not all(sheet in xls.sheet_names for sheet in required_sheets):
        st.error("❌ Mauvais fichier Excel (feuilles manquantes)")
        st.stop()
    ...
    st.error("❌ Erreur de lecture du fichier. Remplace-le via l’admin.")
    st.stop()
